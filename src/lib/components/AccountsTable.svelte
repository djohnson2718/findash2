...
<table class="custom-table"><tbody>
 {#each Object.values(Category) as category (category.id)}
   <tr class="category-table-line"><td >{category.name}</td> <td>${currentBalances.filter(b => b.account.categoryId === category.id)
            .map(b => b.amount).reduce((a, b) => a + b, 0).toLocaleString()}</td></tr>
    {#each currentBalances.filter(b => b.account.categoryId === category.id) as b}
      <tr>
        <td><a class="cell-link" href={`/accounts/${b.account.id}`}>{b.account.name}</a></td>
